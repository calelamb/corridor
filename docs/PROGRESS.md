...
| 1 Foundation | Design draft ready for review; implementation not started | [Foundation design](superpowers/specs/2026-09-20-foundation-design.md). Written-spec review and implementation plan still required by the applied brainstorming workflow. |
...
Review the foundation design, then write the Phase 1 implementation plan with explicit TDD and acceptance tasks. Check current Go/Svelte/tool versions and official dependency documentation before pinning them. Resolve the proposed POST route endpoint and OpenAPI generator compatibility in ADRs before implementing them.
